...
use sift_backend::{BackendHandle, Command, Event};
...
use crate::state::{AppAction, ConnectionState, PendingConnect};
use crate::ui::connect_dialog::{ConnectDialog, DialogAction};
use crate::ui::tabs::{TabId, TabViewerCtx};
use crate::ui::{connect_dialog, log_panel, tree_panel};
    ...
    fn apply_event(&mut self, event: Event) {
        match event {
            ...
            Event::Disconnected { ns } => {
                if matches!(&self.conn,
                    ConnectionState::Connected { profile_id, .. }
                    | ConnectionState::Connecting { profile_id, .. } if *profile_id == ns)
                {
                    self.conn = ConnectionState::Disconnected;
                }
            }
    ...
    fn run_action(&mut self, action: AppAction) {
        match action {
            ...
            AppAction::Disconnect => {
                if let ConnectionState::Connected { profile_id, .. }
                | ConnectionState::Connecting { profile_id, .. } = &self.conn
                {
                    self.backend.send(Command::Disconnect { ns: *profile_id });
                }
            }
...
impl eframe::App for SiftApp {
    fn ui(&mut self, ui: &mut egui::Ui, _frame: &mut eframe::Frame) {
        ...
        egui::Panel::left("tree")
            ...
            .min_size(180.0)
            .show(ui, |ui| {
                tree_panel::show(ui, &self.conn, &mut actions);
            });
